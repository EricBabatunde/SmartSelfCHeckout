...
void SystemDisplay::showInstruction(String msg) {
  while(msg.length() < 20) msg += " "; // Pad to clear the line
  lcd.setCursor(0, 1); // Overwrite the Item Name line temporarily
  lcd.print(msg);
}
...
void SystemDisplay::showCartItem(Item item, int index, int totalItems) {
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Cart Item " + String(index + 1) + "/" + String(totalItems));
  ...
  lcd.setCursor(0, 2);
  lcd.print("Price: $" + String(item.price, 2));
}

void SystemDisplay::showMode(String modeName) {
  while(modeName.length() < 20) modeName += " ";
  lcd.setCursor(0, 0);
  lcd.print(modeName);
}